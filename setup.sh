...
if command -v nvidia-smi >/dev/null 2>&1; then
  ...
  pip install --upgrade pip
  pip install "onnxruntime-gpu[cudnn]" nibabel scipy batchgenerators tkhtmlview markdown
else
  echo "CUDA not detected, installing CPU version of onnxruntime..."
  pip install --upgrade pip
  pip install onnxruntime nibabel scipy batchgenerators tkhtmlview markdown
fi
